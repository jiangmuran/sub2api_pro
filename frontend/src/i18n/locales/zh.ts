export default {
  ...
  voiceChat: {
    ...
    checks: {
      title: '连接检查',
      description: '开始对话前会检查服务端语音能力、浏览器网络和麦克风环境。',
      server: '服务端语音能力',
      serverHint: '检查当前 Key 是否具备语音功能权限。',
      browser: '浏览器网络连通性',
    ...
    log: {
      ...
      preflightSuccess: '服务端能力与浏览器检查已完成。',
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportTitle: '导入账号 / 兼容接口',
      dataImportHint: '上传 JSON 文件以批量导入账号；兼容 sub2api 导出格式和 OpenAI Compatible 导入格式。',
      dataImportSupportedFormats:
        '支持两种格式：1）sub2api 导出 JSON；2）显式声明 `type: "openai-compatible-import"` 的兼容接口 JSON。兼容格式会自动转换为 OpenAI API Key 账号，并默认启用更稳妥的透传配置。',
      dataImportWarning: '导入会创建新账号；兼容接口的实际可用性仍取决于上游实现质量，分组需手工绑定，请先确认不会与现有数据冲突。',
      dataImportFile: '数据文件',
      ...
      batchTest: {
        ...
        delay: '请求延时',
        delayHint: '每个请求之间的延时，避免触发限流',
        concurrency: '并发数',
        concurrencyHint: '同时测试的账号数量',
        timeout: '超时时间',
        start: '开始测试',
        stop: '停止测试',
        testing: '测试中...',
        ...
        statusCode: '状态码',
        export: '导出结果',
        confirmStop: '确定要停止测试吗？',
        cancelled: '测试已取消',
        completeMessage: '批量测试完成，共测试 {count} 个账号',
        error: '批量测试失败'
      },
      ...
      openai: {
        ...
        compatCheckFailedToast: '兼容性检测失败',
        compatOverallStatus: '整体状态',
        compatRecommendedMode: '推荐模式',
        compatStatusCompatible: '完全兼容',
        compatStatusPartial: '基本兼容',
        compatStatusLegacyOnly: '仅旧接口可用',
        compatStatusIncompatible: '不建议接入',
        ...
        compatCheckUnsupported: '不支持',
        compatCheckFailed: '失败',
        compatNoDetail: '暂无详细信息',
        compatAppliedHint: '已根据检测结果应用推荐配置：{mode}。你仍可继续手动调整透传与模型映射。',
        compatBaseUrlRequired: '请先填写 Base URL',
        compatApiKeyRequired: '请先填写 API Key',
        workbenchTitle: '测试模型',
        workbenchDesc: '在线查看兼容源模型、预估价格，并直接做多轮对话预览。',
        refreshWorkbench: '在线获取',
        workbenchEmpty: '先填写 Base URL 和 API Key，再从这里在线获取模型列表。',
        workbenchModels: '模型',
        workbenchPricing: '定价',
        workbenchChat: '对话',
        previewApiKeyTitle: '预览 API Key',
        previewApiKeyPlaceholder: '输入用于当前对话预览的上游 API Key',
        previewApiKeyHint: '这里的 Key 会直接用于当前上游多轮对话预览；默认可一键回填账号 Key。',
        useAccountKey: '使用账号 Key',
        accountKeyApplied: '已填入账号 API Key',
        userTestApiKeyTitle: '用户测试 API Key',
        userTestApiKeyPlaceholder: '可粘贴已有用户 Key，或一键生成新的测试 Key',
        generateUserTestApiKey: '一键生成',
        userTestApiKeyHint: '生成的 Key 会自动复制，可用于保存账号后从用户侧回测网关和实际定价。',
        userTestApiKeyGenerated: '已生成并复制用户测试 API Key',
        userTestApiKeyGenerateFailed: '生成用户测试 API Key 失败',
        userTestApiKeyCopied: '已复制用户测试 API Key',
        onlineModelsTitle: '在线模型列表',
        onlineModelsDesc: '直接读取当前上游返回的模型，适合兼容源和老接口。',
        applyOnlineModels: '填入在线模型',
        noOnlineModels: '暂未获取到模型列表',
        onlineModelsApplied: '已填入在线模型列表',
        modelSummary: '概览',
        modelCount: '模型数量',
        pricedCount: '有价模型',
        ...
        chatPlaceholder: '输入你的消息，支持连续多轮上下文。',
        chatHint: '当前预览优先走兼容源 chat/completions 能力。',
        chatSending: '发送中...',
        sendChat: '发送消息',
        chatFailed: '多轮对话预览失败',
        emptyReply: '上游返回了空响应',
        workbenchLoadFailed: '获取在线模型失败',
      ...
      functionKeyPlaceholder: '用于玩法接口的 Bearer Key',
      functionKeyHint: '仅在接入 `/v1/function/*`（如语音对话）时需要，可留空。',
    ...
    redeem: {
      ...
      days: '天',
      // 邀请码统计与归因
      invitationStatsHint: '邀请码用于跟踪注册来源，下面的统计可以帮助你了解使用情况。',
      invitationTotal: '邀请码总数',
      invitationUsed: '已使用邀请码数',
      invitationUnused: '未使用邀请码数',
      viewInvitationImpact: '查看转化效果',
      invitationImpactTitle: '邀请码转化分析',
      invitationCodeLabel: '邀请码',
      invitationImpactEmpty: '当前邀请码暂无统计数据。',
      registeredUser: '注册用户',
      subscriptionRedeemsTotal: '该用户使用的订阅兑换码总数',
      invitationCategory: '邀请码分类',
      byRedeemCategory: '按兑换码分类统计',
      bySubscriptionGroup: '按订阅分组统计',
      redeemCount: '兑换次数',
      noSubscriptionRedeems: '该用户尚未使用任何订阅兑换码',
      redeemDetails: '兑换明细',
      status: {
        unused: '未使用',
        used: '已使用',
    ...
    distributor: {
      ...
      validityDays: '有效期（天）',
      cost: '成本',
      selectAllOffers: '全选商品',
      bulkUpdate: '批量更新',
      bulkValidityPlaceholder: '批量设置有效期（可选）',
      bulkCostPlaceholder: '批量设置成本 CNY（可选）',
      bulkSelectionRequired: '请先选择要批量更新的商品',
      bulkFieldsRequired: '请至少填写有效期或成本中的一个',
      bulkUpdated: '已更新 {count} 个商品',
      searchOrders: '搜索订单...',
      revoke: '撤销',
      code: '兑换码',
      status: '状态',
    ...
    settings: {
      ...
      site: {
        ...
        contactInfoHint: '填写群聊邀请链接，将展示在兑换页、个人资料和用户菜单中，并以新标签页打开',
        docUrl: '文档链接',
