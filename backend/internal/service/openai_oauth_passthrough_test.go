...
func TestOpenAIGatewayService_OAuthPassthrough_StreamKeepsToolNameAndBodyNormalized(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-5.2","stream":true,"store":true,"service_tier":"auto","stream_options":{"include_usage":true},"metadata":{"user_id":"session_abc"},"instructions":"local-test-instructions","input":[{"type":"text","text":"hi"}]}`)
