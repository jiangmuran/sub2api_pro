...
func (s *TokenRefreshService) processOpenAIOAuthRecovery(ctx context.Context) {
	...
	if err != nil {
		slog.Warn("token_refresh.openai_oauth_recovery_list_failed", "error", err)
		return
	}
	if len(accounts) == 0 {
		return
	...
	for i := range accounts {
		...
		if err != nil {
			slog.Warn("token_refresh.openai_oauth_mark_refreshing_failed", "account_id", account.ID, "error", err)
			continue
		}
		if !ok {
			continue
		...
		if err := s.accountRepo.Update(ctx, account); err != nil {
			slog.Warn("token_refresh.openai_oauth_update_failed", "account_id", account.ID, "error", err)
			s.rollbackOpenAIOAuthRecoveryState(ctx, account.ID, err)
			failed++
			continue
		}
		if s.cacheInvalidator != nil {
			if err := s.cacheInvalidator.InvalidateToken(ctx, account); err != nil {
				slog.Warn("token_refresh.openai_oauth_cache_invalidate_failed", "account_id", account.ID, "error", err)
			}
		}
		if s.schedulerCache != nil {
			if err := s.schedulerCache.SetAccount(ctx, account); err != nil {
				slog.Warn("token_refresh.openai_oauth_scheduler_cache_failed", "account_id", account.ID, "error", err)
			}
		}
		refreshed++
	}
	...
	if refreshed > 0 || failed > 0 {
		slog.Info("token_refresh.openai_oauth_recovery_complete", "candidates", len(accounts), "refreshed", refreshed, "failed", failed)
	}
}

func (s *TokenRefreshService) rollbackOpenAIOAuthRecoveryState(ctx context.Context, accountID int64, err error) {
	...
	if upErr := s.accountRepo.UpdateOpenAIOAuthState(ctx, accountID, update); upErr != nil {
		slog.Warn("token_refresh.openai_oauth_rollback_failed", "account_id", accountID, "error", upErr)
	}
}
...
func (s *TokenRefreshService) handleOpenAIOAuthRecoveryFailure(ctx context.Context, account *Account, err error) {
	...
	if attempts > OpenAIOAuthRefreshMaxAttempts {
		...
		if upErr := s.accountRepo.UpdateOpenAIOAuthState(ctx, account.ID, update); upErr != nil {
			slog.Warn("token_refresh.openai_oauth_state_update_failed", "account_id", account.ID, "error", upErr)
		}
		...
		if setErr := s.accountRepo.SetError(ctx, account.ID, setErrMsg); setErr != nil {
			slog.Warn("token_refresh.openai_oauth_set_error_failed", "account_id", account.ID, "error", setErr)
		}
		return
	}
