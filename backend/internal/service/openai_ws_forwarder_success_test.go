...
func TestOpenAIGatewayService_Forward_WSv2_OAuthStoreFalseByDefault(t *testing.T) {
	...
	require.Equal(t, "message", gjson.Get(requestJSON, "input.0.type").String(), "OAuth WSv2 顶层文本输入应包装为 message")
	require.Equal(t, "user", gjson.Get(requestJSON, "input.0.role").String())
	require.Equal(t, "input_text", gjson.Get(requestJSON, "input.0.content.0.type").String())
	require.Equal(t, "hello", gjson.Get(requestJSON, "input.0.content.0.text").String())
