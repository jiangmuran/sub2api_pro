...
func (r *accountRepository) TrySetOpenAIOAuthRefreshing(ctx context.Context, id int64, now time.Time) (bool, error) {
	...
	if rows > 0 {
		if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
			logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue oauth refreshing failed: account=%d err=%v", id, err)
		}
		r.syncSchedulerAccountSnapshot(ctx, id)
	}
	return rows > 0, nil
...
func (r *accountRepository) UpdateOpenAIOAuthState(ctx context.Context, id int64, update service.OpenAIOAuthStateUpdate) error {
	...
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
		logger.LegacyPrintf("repository.account", "[SchedulerOutbox] enqueue oauth state update failed: account=%d err=%v", id, err)
	}
	r.syncSchedulerAccountSnapshot(ctx, id)
	return nil
}
