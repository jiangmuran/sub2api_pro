...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, settingService, compositeTokenCacheInvalidator, openAIOAuthService, opsRepository)
	...
	securityChatRepository := repository.NewSecurityChatRepository(db)
	securityChatService := service.NewSecurityChatService(securityChatRepository, settingRepository)
	securityChatAIService := service.NewSecurityChatAIService(settingService, apiKeyService, configConfig)
	securityHandler := admin.NewSecurityHandler(securityChatService, securityChatAIService, apiKeyService)
	distributorHandler := admin.NewDistributorHandler(distributorService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, securityHandler, distributorHandler)
	...
	handlerDistributorHandler := handler.NewDistributorHandler(distributorService)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerDistributorHandler)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, securityChatService, settingService, redisClient)
